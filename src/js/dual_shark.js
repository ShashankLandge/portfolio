...
function updateBodySegments(shark) {
  ...
  const segmentDistance = 6 * scaleFactor;
  ...
  for (let i = 1; i < shark.bodySegments.length; i++) {
    ...
    const waveOffset =
      Math.sin(shark.time * waveFrequency + i * 0.8) * waveAmplitude;
...
function enqueueBezierPaths(
  shark,
  count = 3,
  start = { x: shark.x, y: shark.y }
) {
