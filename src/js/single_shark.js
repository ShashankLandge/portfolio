...
let scaleFactor = Math.min(Math.max(window.innerWidth / 1400, 0.5), 1);
...
function updateParticles() {
  ...
  for (let i = 0; i < particles.length; i++) {
    ...
    if (distance < avoidanceRadius && distance > 0) {
      ...
      const avoidanceStrength = Math.pow(
      ...
      particle.avoidanceX =
        normalizedDx * avoidanceStrength * maxAvoidanceForce;
      particle.avoidanceY =
        normalizedDy * avoidanceStrength * maxAvoidanceForce;
    } else {
...
function updateBodySegments() {
  ...
  const segmentDistance = 6 * scaleFactor; // Fixed distance between segments
  const waveFrequency = 0.015;
  const waveAmplitude = (dragging ? 1.5 : 0.8) * scaleFactor;
  ...
  for (let i = 1; i < fish.bodySegments.length; i++) {
    ...
    const waveOffset =
      Math.sin(fish.time * waveFrequency + i * 0.8) * waveAmplitude;
...
/*
function drawCursor() {
  if (!mouseReady || mouse.visible === false) return;
...
window.addEventListener("resize", () => {
  canvas.width = window.innerWidth;
  canvas.height = window.innerHeight;
  scaleFactor = Math.min(Math.max(window.innerWidth / 1400, 0.5), 1);
  fish.pathQueue = [];
  fish.currentCurve = null;
  enqueueBezierPaths(3, { x: fish.x, y: fish.y });
