import { SHARK, SHARK_TYPE, BEZIER, SharkTypeName } from "./constants";
import {
  BezierCurve,
  Point,
  bezierPoint,
...
export type BodySegment = { x: number; y: number; angle: number };
export type TrailSample = { x: number; y: number; age: number };
...
export type SharkUpdateContext = {
  mouse: MouseState;
  isDragging: boolean;
  canvasWidth: number;
  canvasHeight: number;
...
export type SharkConstructorArgs = {
  type: SharkTypeName;
  x: number;
  y: number;
  scaleFactor: number;
...
export class Shark {
  ...
  time: number;
  idleT: number;
  pathQueue: BezierCurve[];
  currentCurve: BezierCurve | null;
  bodySegments: BodySegment[];
  opacity: number;
  targetOpacity: number;
  trail: TrailSample[];
  ticksSinceTrailSample: number;
  ...
  initBodySegments(scaleFactor: number): void {
    this.bodySegments = [];
  ...
  enqueueBezierPaths(
    canvasWidth: number,
    canvasHeight: number,
    count: number = BEZIER.INITIAL_QUEUE_COUNT,
    startPoint: Point | null = null
  ): void {
    let cursor: Point = startPoint ?? { x: this.x, y: this.y };
  ...
  resetPaths(canvasWidth: number, canvasHeight: number): void {
    this.pathQueue = [];
    this.currentCurve = null;
    this.idleT = 0;
    this.enqueueBezierPaths(
      canvasWidth,
      canvasHeight,
      BEZIER.INITIAL_QUEUE_COUNT,
      { x: this.x, y: this.y }
    );
  }
  ...
  followMouse(mouse: MouseState): void {
  ...
  followBezierPath(canvasWidth: number, canvasHeight: number): void {
    if (this.pathQueue.length < BEZIER.PATH_QUEUE_MIN) {
      this.enqueueBezierPaths(
        canvasWidth,
        canvasHeight,
        BEZIER.PATH_QUEUE_REFILL_COUNT
      );
    }

    if (!this.currentCurve) {
      this.currentCurve = this.pathQueue.shift() ?? null;
    }
  ...
  updateBodySegments(scaleFactor: number, isDragging: boolean): void {
    const segmentDistance = SHARK.SEGMENT_DISTANCE * scaleFactor;
    const baseAmplitude =
      (isDragging
        ? SHARK.WAVE_AMPLITUDE_DRAGGING
        : SHARK.WAVE_AMPLITUDE_IDLE) * scaleFactor;
    ...
    for (let i = 1; i < this.bodySegments.length; i++) {
      ...
      currentSegment.x +=
        (targetX - currentSegment.x) * SHARK.SEGMENT_LERP_FACTOR;
      currentSegment.y +=
        (targetY - currentSegment.y) * SHARK.SEGMENT_LERP_FACTOR;
      ...
      currentSegment.angle +=
        segmentAngleDiff * SHARK.SEGMENT_ANGLE_LERP_FACTOR;
    }
  }
  ...
  drawHead(ctx: CanvasRenderingContext2D, scaleFactor: number): void {
    ctx.save();
  ...
  drawBodyLine(ctx: CanvasRenderingContext2D, scaleFactor: number): void {
  ...
  drawRearFins(ctx: CanvasRenderingContext2D, scaleFactor: number): void {
  ...
  drawTail(ctx: CanvasRenderingContext2D, scaleFactor: number): void {
  ...
  tracePathRegularHead(ctx: CanvasRenderingContext2D): void {
    ctx.beginPath();
    ctx.moveTo(22, 0);
  ...
  tracePathHammerhead(ctx: CanvasRenderingContext2D): void {
    ctx.beginPath();
    ctx.moveTo(16, -14);
    ctx.lineTo(18, -14);
    ctx.lineTo(20, -10);
  ...
  tracePathSideFins(ctx: CanvasRenderingContext2D): void {
    ctx.beginPath();
    ctx.moveTo(0, -6);
    ctx.lineTo(-8, -15);
    ctx.lineTo(-4, -5);
  ...
  tickOpacity(): void {
    if (this.targetOpacity > this.opacity) {
      this.opacity = Math.min(
        this.targetOpacity,
        this.opacity + SHARK.FADE_IN_RATE
      );
    } else if (this.targetOpacity < this.opacity) {
      this.opacity = Math.max(
        this.targetOpacity,
        this.opacity - SHARK.FADE_OUT_RATE
      );
    }
  }

  isFullyFadedOut(): boolean {
